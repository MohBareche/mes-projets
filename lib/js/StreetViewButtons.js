L.StreetView = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    map.on(
      "moveend",
      function () {
        if (!this._fixed) this._update(map.getCenter());
      },
      this
    );
  ...
  _addProvider: function (provider) {
    if (!this.options[provider[0]]) return;
    if (provider[0] == "mapillary" && !this.options.mapillaryId) return;
              ...
              function (data) {
                if (data && data.osv && data.osv.sequences) {
                  var seq = data.osv.sequences[0],
                    url =
                      "https://www.openstreetcam.org/details/" +
                      seq.sequence_id +
                      "/" +
                      seq.sequence_index;
                  window.open(url, button.target);
  ...
  _update: function (center) {
    ...
    for (var i = 0; i < this._buttons.length; i++) {
      ...
      if (b._needUrl) b.href = tmpl;
      else b._href = tmpl;
    }
  },

  _ajaxRequest: function (url, callback, post_data) {
    if (window.XMLHttpRequest === undefined) return;
